...
export async function GET(request: NextRequest) {
  ...
  if (rol === "JUZGADO") { solicitudesQuery = solicitudesQuery.eq("nombre_juzgado", session.user.nombreJuzgado || juzgadoFiltro || "") }
  ...
  for (const s of rows) {
    ...
    radMap[juz].total_radicadas++; radMap[juz].pendientes++
    if (s.abogado_asignado_id) radMap[juz].asignadas_abogado++
  }
  ...
  const activas = rows.filter((s: any) => s.estado !== "RADICADA_EN_GCC")
  const cerradas = rows.filter((s: any) => s.estado === "RADICADA_EN_GCC")
  ...
  const montoTotalCerrado = cerradas.reduce((sum: number, s: any) => sum + extraerMonto(s.etapa_preliminar?.cantidad), 0)
  ...
  const cerradasConFechas = cerradas.filter((s: any) => s.fecha_solicitud); let sdc = 0
  for (const s of cerradasConFechas) { sdc += (Date.now() - new Date(s.fecha_solicitud).getTime()) / 86400000 }
  const kpi = { montoTotalPendiente, totalSancionados, totalRadicaciones: rows.length, totalActivas: activas.length, totalCerradas: cerradas.length, valorTotalSancionesCOP, valorSancionesPendienteCOP, valorSancionesCerradoCOP, tiempoPromedioCierre: cerradasConFechas.length > 0 ? Math.round((sdc / cerradasConFechas.length) * 10) / 10 : 0, tasaRecaudo: montoTotalCerrado + montoTotalPendiente > 0 ? Math.round((montoTotalCerrado / (montoTotalCerrado + montoTotalPendiente)) * 1000) / 10 : 0 }
  ...
  const { data: logsRaw } = await supabase.from("logs_auditoria").select("usuario_id, tipo_accion")
  const { data: usuarios } = await supabase.from("usuarios").select("id, rol")
  const rolPorUsuario: Record<string, string> = {}; for (const u of (usuarios || [])) { rolPorUsuario[u.id] = u.rol }
  const eficienciaPorRol: Record<string, any> = { JUZGADO: { total_acciones: 0, roles_especificos: {} }, GESTOR: { total_acciones: 0, roles_especificos: {} }, ABOGADO: { total_acciones: 0, roles_especificos: {} }, ADMIN: { total_acciones: 0, roles_especificos: {} } }
  for (const log of (logsRaw || [])) { const r = rolPorUsuario[log.usuario_id] || "DESCONOCIDO"; if (eficienciaPorRol[r]) { eficienciaPorRol[r].total_acciones++; eficienciaPorRol[r].roles_especificos[String(log.tipo_accion || "OTRO")] = (eficienciaPorRol[r].roles_especificos[String(log.tipo_accion || "OTRO")] || 0) + 1 } }
